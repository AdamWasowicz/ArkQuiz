...
export const getAllRaces = (): string[] => {
    ...
    headers.forEach((item) => 
        {
            const operator = getOperatorById(item.Id);
            if (operator !== undefined) {
                const race: string | string[] = operator.Race;
...
export const doHealthCheck = (): OperatorHealthcheckResult => {
    const output: OperatorHealthcheckResult = {
        ...
        ErrorsIcon: __doHealthCheck_Icon(),
        ErrorsRace: __doHealthCheck_Race()
    }
...
const __doHealthCheck_Operator = (): string[] => {
    ...
    allOperatorHeaders.forEach((header) => {
        const operatorData: Operator | undefined = getOperatorById(header.Id);
